	...
	rshp::nodes::MixColors, rshp::nodes::RandomColorSource, rshp::nodes::Timer, rshp::nodes::ValueToColor)
{
	GIVEN("Node type '" + TestType::name + "'")
	{
		...
		{
			auto& nodeMap = rshp::base::AbstractNode::getNodeMap();
			REQUIRE(nodeMap.find(TestType::name) != nodeMap.end());
			AND_THEN("it is constructible from the nodemap factory")
			{
				std::unique_ptr<rshp::base::AbstractNode> node = nullptr;
				REQUIRE_NOTHROW(node = std::move(rshp::base::AbstractNode::createNode(TestType::name)));
				...
				AND_THEN("its name is available from the base class")
				{
					...
					if constexpr(std::is_base_of_v<rshp::base::AbstractSink, TestType>)
					{
						...
						{
							auto sink = dynamic_cast<rshp::base::AbstractSink*>(node.get());
					...
					if constexpr(std::is_base_of_v<rshp::base::AbstractSource, TestType>)
					{
						...
						{
							auto source = dynamic_cast<rshp::base::AbstractSource*>(node.get());
...
TEST_CASE("base::nodes::Graph update behaviour")
{
	GIVEN("Source A with one output port, AO, and update event AU")
	{
		...
		auto& AU = A.getOutputEventPort(rshp::base::AbstractNode::OutputEvents::Updated);

		AND_GIVEN("test sink B with one input port, BI, and update event BU")
		{
			TestSink B;
			auto& BI = B.getInputDataPort<TestSink::InputPorts::Message>();
			auto& BU = B.getOutputEventPort(rshp::base::AbstractNode::OutputEvents::Updated);
			...
			AND_GIVEN("test node C with one input port, CI and two output ports, CO1 and CO2, and update event CU")
			{
				...
				auto& CU = C.getOutputEventPort(rshp::base::AbstractNode::OutputEvents::Updated);
				...
				WHEN("AO is connected to CI, CO1 is connected to BI")
				{
					...
					AND_GIVEN("test sink D with one input port DI and an update event DU")
					{
						...
						auto& DU = D.getOutputEventPort(rshp::base::AbstractNode::OutputEvents::Updated);
