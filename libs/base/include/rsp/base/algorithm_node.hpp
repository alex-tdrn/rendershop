...
public:
	...
	explicit algorithm_node(std::unique_ptr<rsp::algorithm>&& algorithm);
	...
	void set_algorithm(std::unique_ptr<rsp::algorithm>&& algorithm);
	auto get_input_ports() const -> std::vector<rsp::input_port*> final;
	auto get_output_ports() const -> std::vector<rsp::output_port*> final;
	void pull(std::weak_ptr<rsp::sentinel> const& sentinel = {}) final;
	void push(std::weak_ptr<rsp::sentinel> const& sentinel = {}) final;
...
inline algorithm_node::algorithm_node(std::unique_ptr<rsp::algorithm>&& algorithm)
{
...
inline void algorithm_node::set_algorithm(std::unique_ptr<rsp::algorithm>&& algorithm)
{
...
inline auto algorithm_node::get_input_ports() const -> std::vector<rsp::input_port*>
{
	if(algorithm == nullptr)
		return {};
...
inline auto algorithm_node::get_output_ports() const -> std::vector<rsp::output_port*>
{
	if(algorithm == nullptr)
		return {};
...
inline void algorithm_node::pull(std::weak_ptr<rsp::sentinel> const& sentinel)
{
	if(sentinel_present() || !update_possible())
		return;

	std::shared_ptr<rsp::sentinel> sentinel_origin;
	if(sentinel.expired())
	{
		sentinel_origin = std::make_shared<rsp::sentinel>();
...
inline void algorithm_node::push(std::weak_ptr<rsp::sentinel> const& sentinel)
{
	if(sentinel_present() || !update_possible())
		return;
